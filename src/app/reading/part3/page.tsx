...
function ReadingPart3Content() {
  ...
  useEffect(() => {
    if (groupId && groupId >= 1 && groupId <= READING_PART_3.length) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('reading_part3_completed', JSON.stringify(completedGroups));
    } catch (error) {
  ...
  useEffect(() => {
    const currentExercise = READING_PART_3[currentGroup];
    if (currentExercise) {
  ...
  const checkAnswers = () => {
    ...
    const allAnswered = currentExercise.data.every(item => userAnswers[item.key]);
    if (!allAnswered) {
      alert("Please match all paragraphs with headings before checking answers.");
      return;
    }
    
    // Check correctness
    const correctAnswers = currentExercise.data.filter(item => 
      userAnswers[item.key] === item.option
    ).length;
  ...
  const nextGroup = () => {
    if (currentGroup < READING_PART_3.length - 1) {
  ...
  return (
    <div className="gradient-bg" style={{ display: "flex", minHeight: "100vh", fontFamily: "Inter, system-ui, sans-serif" }}>
      ...
      >
        ...
        >
          {READING_PART_3.map((group, index) => {
        ...
        <div className="card" style={{ marginTop: 24, padding: 16 }}>
          ...
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ fontSize: "14px", color: "var(--card-text)" }}>
              / {READING_PART_3.length} Completed
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="main-content" style={{ flex: 1, padding: 20 }}>
        ...
        <div className="card" style={{ marginBottom: 24 }}>
          ...
          <ul style={{ color: "var(--card-text)", lineHeight: 1.6, margin: 0, paddingLeft: 20 }}>
            ...
            <li>Choose the most suitable heading for each paragraph from the list provided</li>
        ...
        <div style={{ marginBottom: 24 }}>
          {currentExercise.data.map((item, index) => {
            ...
            const isIncorrect = showAnswers && userAnswer && userAnswer !== item.option;
            ...
            return (
              <div
                ...
                style={{ 
                  marginBottom: 20,
                  border: showAnswers ? 
                    (isCorrect ? "2px solid #10b981" : isIncorrect ? "2px solid #ef4444" : "1px solid var(--border-color)") :
              ...
              >
                <div style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ marginBottom: 12 }}>
                    ...
                    {userAnswer ? (
                      <div className="selected-heading" style={{
                        padding: "16px 20px",
                        ...
                        <span style={{ color: "#ffffff", fontWeight: "600", fontSize: "16px", lineHeight: "1.5" }}>
                          {availableHeadings.findIndex(h => h === userAnswer) !== -1 && 
                            String.fromCharCode(65 + availableHeadings.findIndex(h => h === userAnswer))
                          }. {userAnswer}
                        </span>
                        {!showAnswers && (
                          <button
                      ...
                      <div className="heading-options-container" style={{
                        display: "grid",
                        ...
                        {availableHeadings.map((heading, headingIndex) => {
                          const isUsedByOther = Object.values(userAnswers).includes(heading);
                          const optionLetter = String.fromCharCode(65 + headingIndex);
                          
                          return (
                            <button
                              key={headingIndex}
                              onClick={() => !isUsedByOther && !showAnswers && handleHeadingSelect(item.key, heading)}
                              disabled={isUsedByOther || showAnswers}
                              className="option-button"
                              style={{
                                padding: "12px 16px",
                                border: `2px solid ${isUsedByOther ? "#d1d5db" : "var(--border-color)"}`,
                                borderRadius: "8px",
                                backgroundColor: isUsedByOther ? "#f9fafb" : "var(--card-background)",
                                color: isUsedByOther ? "#6b7280" : "var(--foreground)",
                            ...
                            >
                              <span style={{ fontWeight: "bold", color: "#3b82f6", marginRight: "8px" }}>
                                {optionLetter}.
                              </span>
                              <span>{heading}</span>
                            </button>
            ...
            <div style={{ textAlign: "center", marginBottom: 16 }}>
              <h3 style={{ 
                ...
                {isCompleted ? 
                  ...
                  `📝 ${currentExercise.data.filter(item => userAnswers[item.key] === item.option).length} out of ${currentExercise.data.length} correct`
                }
              </h3>
            </div>
          </div>
        ...
        <div style={{ 
          ...
          <button 
            onClick={nextGroup} 
            disabled={currentGroup === READING_PART_3.length - 1}
            className="btn btn-secondary"
            style={{ minWidth: 120 }}
          >
            Next ➡️
